-- Script SQL simplificado para crear las tablas en Supabase
-- Ejecutar este script primero en el SQL Editor de Supabase
...
BEGIN
    ...
    INSERT INTO public.usuario (usuario_id, rol_id, nombres, apellidos, email, activo)
    VALUES (
        NEW.id,
        admin_role_id,
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE WARNING 'Error creating user in usuario table: %', SQLERRM;
        RETURN NEW;
END;
...
-- Política para insertar usuarios (necesaria para el trigger)
CREATE POLICY "Allow insert for trigger" ON usuario
